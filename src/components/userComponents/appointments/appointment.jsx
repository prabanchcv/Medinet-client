  ...
  useEffect(() => {
    async function datacall() {
      if (!docData.doctorData) {
        ...
        await axios
          .get(import.meta.env.VITE_BASE_URL + `docSchedule/${docData._id}`, {
            headers: {
      ...
      <div className=" slice mx-auto text-center mt-5 p-4 app-div" style={{ maxWidth: '800px' }}>
        ...
        <div className="row">
            ...
            <div className="row">
              ...
              <div className="col-12 text-center">
                ...
                <div className="text-secondary">
                  {docData.doctorData ? docData?.doctorData[0]?.name : ''}
                </div>
                <div className="text-secondary">{docData?.qualification}</div>
                <div>
                  <b>
                    Fee : <span className="text-success">{docData.fee}</span>
                  </b>
                </div>
                <div className="">
                  <b>Session Timing </b>
                  ...
                  <div className="dropdown">
                    <button
                      className="btn drpp p-1 bg-light btn-outline-dark mt-2  text-dark dropdown-toggle"
                      style={{ maxWidth: '150px', minWidth: '100px', fontSize: '14px' }}
                      type="button"
                    ...
                    <ul className="dateDrop dropdown-menu">
                      {timeList &&
                        timeList.map((el, index) => (
                          <li key={index}>
                            <Link className="appLink" onClick={() => setSessionTime(el)}>
                              {el}
                            </Link>
                          </li>
                        ))}
          ...
          <div className="col-12 mt-4 d-flex justify-content-center align-items-center">
            <button className="btn p-1 btn-outline-success" onClick={handleSubmit}>
              Book Appointment <BsPlusLg style={{ marginTop: '-2px' }} />
            </button>
          </div>
        </div>
      </div>
